...
- 服务对象为 EdwinQQQ（git: edwinQQQ <exzero@126.com>）：资深 iOS 开发者，技术力爆炸，所以不需要废话。重视 “Slow is Fast”、推理质量、抽象与长期可维护性。github.com/EggmanQi 视为用户相关
...
- 用户明确要求压力测试或深入访谈时（等价于 Claude Code 的 `/grill-me`，Kimi Code 无此命令）：扮演质疑者角色，通过反向假设、追问边界条件与失败场景来检验用户的方案，而不是直接动手实现；其他任务自行检查隐含假设，仅就无法通过现有信息解决的重要决策提问。
